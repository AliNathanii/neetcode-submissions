class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i, a in enumerate(nums):  # i will be index value and a will be the int at that index posittion.
            if i > 0 and a == nums[i-1]:  # i > 0 meaning this is not the first value in input array if a is equal to the previous value then we will continue to the next iteration of the loop. This is done to not reuse the same int from nums.
