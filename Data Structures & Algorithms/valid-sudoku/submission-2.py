class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        squares = defaultdict(list)  # key for this one will be a pair of row/3 and c/3 ie (r/3, c/3)
        ...
        for r in range(9):  # iterating over rows with index r.
            for c in range(9):  # iterating over column now with index c.
                ...
                cols[c].append(board[r][c])  # add, to the key cols[c] value boardvalueof[r][c]
                rows[r].append(board[r][c])  # add, to the the key rows[r] value boardvalueof[r][c]
                squares[(r//3, c//3)].append(board[r][c])  # add, to the key r//3, c//3 in hashset squares, the value board[r][c]
        return True
