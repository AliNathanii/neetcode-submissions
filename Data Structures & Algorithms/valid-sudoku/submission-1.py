    ...
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        for r in range(9):  # iterating over rows with index r.
            for c in range(9):  # iterating over column now with index c.
                ...
                cols[c].add(board[r][c])  # add, to the key cols[c] value boardvalueof[r][c]
                rows[r].add(board[r][c])  # add, to the the key rows[r] value boardvalueof[r][c]
                squares[(r//3, c//3)].add(board[r][c])  # add, to the key r//3, c//3 in hashset squares, the value board[r][c]
        return True
